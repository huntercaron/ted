...
const InfoTooltip = styled.div`
  ...
  background-color: white;
  width: 280px;
  box-shadow: 0 2px 34px 0 rgba(0, 0, 0, 0.09), 0 2px 6px 0 rgba(0, 0, 0, 0.1);
...
class IndexPage extends React.PureComponent {
  ...
  fetchTedData = (term = 'applause') => {
    ...
    for (let i = 1; i <= 5; i++) {
      talkRequests.push(
        this.fetchSingle(`ted-section-${i}?search=${searchTerm}`)
      )
    }
  ...
  render() {
    ...
    return (
      <Container>
        ...
        <InfoTooltip ref={el => (this.tooltip = el)} style={tooltipStyles}>
          <MetaContainer>
        ...
        <a
          href={
            this.state.tooltipOpen
              ? this.state.lineLink +
                '#t-' +
                timeToMilliseconds(this.state.lineTime)
              : '#'
          }
